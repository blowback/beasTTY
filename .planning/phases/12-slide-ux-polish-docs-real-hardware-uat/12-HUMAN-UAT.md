...
[Phase 12.1 fixes verified: 3 passed, 2 pending hardware test, 1 blocked on upstream PR]
...
result: pass
note: "Confirmed working post Plan 12-08 (RTS-on-connect fix). Multi-file batch (2 files) transfers cleanly after hard reload. User confirmed 2026-05-09."
...
result: pending
note: "RTS-blocker (Plan 12-08) is resolved — Test 3 SEND now passes. RECV direction with zero-byte file not directly exercised yet. The same RTS fix should unblock recv, but recv has its own state machine and merits an independent hardware run before sign-off."
...
result: pending
note: "Cascade-block resolved by Plan 12-08 (RTS fix); send path now reaches stable active-send state. Mid-send cancel not directly exercised yet — needs an independent hardware run."
