...
Ran 2026-08-06 (`/code-review --fix`, commit `299646f`). **Six findings: four fixed, two recorded as known limits.**

**Fixed — the in-use classifier was reachable from steps that cannot mean "another tab."** `reportOpenFailure(err)` sat on `try` blocks that also covered `setSignals()` (and, on the auto-connect path, `writable.getWriter()` / `registerWriter()`). Per the Web Serial spec `setSignals()` rejects `NetworkError`, which AC-5's widened `isPortInUse` now matches — so a single-adapter user whose device was yanked *after* a successful `open()` was told "That MicroBeast is already connected in another Beastty tab" while this tab was the only one that had ever held it. Callers now track whether `open()` itself resolved and pass `fromOpen` in. Third site: `retryOpenOnce` — AC-5 unified the click and auto-connect paths but left the auto-reconnect re-open on the raw Chromium string, so the exact scenario the story exists for (two tabs, unplug/replug, tab B's reconnect wins) read `Reconnect failed: Failed to execute 'open' on 'SerialPort': Failed to open serial port.` It now shares `PORT_IN_USE_MSG`, deliberately without setting `lastConnectError` — it lands in `port-lost`, whose readout is its own label, and a stale message would resurface on the next `disconnected`.

**Fixed — half-open rollback (a defect underneath the above, found by pulling on it).** `open()` resolving and the connection succeeding are not the same event; `port` is assigned only after `setSignals()` and `getWriter()` have *also* succeeded. A throw in that window left the device open with nothing referencing it — `teardown()` walks `port`, still null — so it stayed open until the page reloaded, locked against every other tab. Sharper still: `handleReconnect` handed that same still-open port to the D-04 retry, whose `open()` then rejects `InvalidStateError`, a name `isPortInUse()` classifies as a cross-tab hold. A signalling fault therefore surfaced 500ms later as an instruction to go find a tab that was never there (reproduced: `reopen-failed: That MicroBeast is already connected in another Beastty tab.`). New `closeHalfOpenPort(p, acquiredWriter)` rolls the half-built connection back on all five paths (`connectMicroBeast`, auto-connect, `handleReconnect`, `retryOpenOnce`, `finishReconnect`). The writer is passed **explicitly** rather than read off module-level `writer`: the reconnect paths can still be holding the *previous* connection's writer (`onNavSerialDisconnect` sets `port-lost` without running teardown) and that one is not theirs to release. `getWriter()` in `connectMicroBeast` and `finishReconnect` moved inside a `try` — a throw there escaped as an unhandled rejection, stranding the port open and the UI on `connecting` with no way back short of a reload.

**Fixed — an AC-2 assertion that proved nothing.** `auto-connect.spec.js` asserted `#menu-connect-item` `data-state="disconnected"`, which `index.html:1716` ships in the static markup — so it resolved before a line of `main.js` had run, and the "nothing opened, nothing logged" assertions after it were racing the boot scan they exist to observe (it flaked once in a full-suite run). All three cases now wait on the multi-adapter cue first, which only the scan can paint.

**Recorded, not fixed — `isPortInUse` treats every `NetworkError` as a cross-tab hold.** Chromium raises the same name and message for any device-acquisition failure, including OS permission denial; on Linux a user not in `dialout`/`uucp` is the most common first-run CP2102N failure and will hunt for a phantom tab. The message is a verbatim AC and the real answer is S11.2's `BroadcastChannel` — this is the second half of the limitation already documented above `isPortInUse`, now written down rather than papered over.

**Recorded, not fixed — the boot cue hardcodes "MicroBeasts" but counts stored-preset matches.** Per correction §(b) the count comes from whatever the user last connected to, so two CH340 clones granted via "Show all serial devices" read as "2 MicroBeasts detected". The single-match branch avoids exactly this by going through `getConnectionDevice()`. Verbatim AC; left alone.

**Tests.** New `www/tests/transport/half-open-rollback.spec.js` (2 cases, both proven red on the unfixed tree with the wrong message quoted above). `mock-serial.js` gained spec-accurate `InvalidStateError` on re-opening an open port — the omission is why nothing had ever caught this — plus a `__forceSetSignalsReject` hook.

**Suite: 748 passed / 1 skipped / 2 flaky.** Neither flake is from this diff: `errors.spec.js:83` passes 120/120 isolated and overrides `p.open` wholesale so it never reaches a changed path. A control run of the full suite on the stashed tree flaked **6**, a different set again — this suite has a standing pool of load-sensitive flakes, worth its own story.

## Change Log

| Date | Change |
|---|---|
| 2026-08-06 | Code review (`--fix`, commit `299646f`) → **done**. Six findings: four fixed, two recorded as known limits (both are verbatim ACs). The classifier was reachable from `setSignals()`/`getWriter()` rejections, which cannot mean "another tab" because this tab already holds the port by then; `retryOpenOnce` was a third `open()` site AC-5 had missed, still dumping the raw Chromium string in exactly the two-tab replug case the story is named for. Pulling on that surfaced a defect underneath it: nothing assigns `port` until `setSignals()` and `getWriter()` have also succeeded, so a throw between them stranded the device open with no reference `teardown()` could reach — and `handleReconnect` then fed that still-open port to the D-04 retry, whose `open()` rejects `InvalidStateError`, which the classifier reads as a cross-tab hold. A signalling fault surfaced 500ms later as an instruction to find a tab that never existed. `closeHalfOpenPort()` on all five paths, writer passed explicitly so a reconnect never releases the previous connection's. Also fixed an AC-2 assertion satisfied by static markup (it resolved before `main.js` ran and raced the scan it meant to observe). New `half-open-rollback.spec.js`, both cases proven red first; mock gained spec-accurate `InvalidStateError` on re-open — the omission is why this had never been caught. Suite 748 passed / 1 skipped / 2 flaky, both from the pre-existing load-sensitive pool (a control run on the unmodified tree flaked 6, a different set). Next: S11.2 (`peer-link.js`). |
