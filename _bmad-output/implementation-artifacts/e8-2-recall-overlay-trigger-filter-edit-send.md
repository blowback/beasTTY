...
- [Source: www/renderer/paste-toast.js] — transient-renderer seam (lifecycle + injected deps + [hidden] toggle + retainFocus + window hook)
...
- `www/renderer/command-history.js` — **NEW.** The recall overlay (transient-renderer, `paste-toast.js` seam; distinct from the E8.1 engine at `www/input/command-history.js`).
