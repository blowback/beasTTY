...
const DEFAULTS = Object.freeze({
    ...
    pasteLineEnding: 'cr',       // Settings ▸ Paste line ending — what every line break in
                                 //   PASTED text is rewritten to before it reaches the wire
                                 //   ('cr' | 'lf' | 'crlf' | 'raw'). Deliberately SEPARATE from
                                 //   crlfMode above: that one governs the Enter key, this one
                                 //   governs paste, and neither reads the other. Clipboard text
                                 //   on Linux arrives LF-only, which the MicroBeast does not
                                 //   accept as a line break — hence the 'cr' default. 'raw'
                                 //   passes the clipboard bytes through untouched. Validated at
                                 //   its consumer (paste-pump.js setPasteLineEnding), as
                                 //   crlfMode is at setCrlfMode — prefs.js has no field
                                 //   validation. CURRENT_VERSION NOT bumped (defensive
                                 //   spread-merge, same precedent as every pref above).
    pasteSpeed: 240,             // Settings ▸ Paste speed — bytes/sec the paste pump aims for,
                                 //   with 0 meaning "as fast as the wire allows" (the pre-fix
                                 //   behaviour, kept for targets that can take it). A full-speed
                                 //   paste writes 32 bytes at a time into a 16-byte UART FIFO,
                                 //   so it overruns a Z80 editor whatever the average rate; at
                                 //   any paced speed the pump drops to 8-byte writes and pauses
                                 //   longer after a line break. 240 is an estimate pending real
                                 //   hardware, not a measurement. Validated at its consumer
                                 //   (paste-pump.js setPasteSpeed). CURRENT_VERSION NOT bumped.
