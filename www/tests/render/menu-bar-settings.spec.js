...
test.describe('Settings ▸ Paste speed', () => {
  test('the row defaults to 240 B/s @fast', async ({ page }) => {
    await ready(page);
    await page.evaluate(() => window.__menuBar.open('settings'));
  ...
  for (const { value, speed, gapMs } of [
    { value: '0', speed: 0, gapMs: 19 },      // 32 B at computeGap(19200)
    { value: '480', speed: 480, gapMs: 17 },  // 8 B every round(8/480*1000)
    { value: '120', speed: 120, gapMs: 67 },
    { value: '60', speed: 60, gapMs: 133 },
  ]) {
    test(`${value} persists AND repaces the pump (gap ${gapMs} ms) @fast`, async ({ page }) => {
      ...
      expect(await page.evaluate(() => window.__pastePump.__getStateForTests().gapMs)).toBe(gapMs);
      expect(await page.evaluate(() => document.activeElement && document.activeElement.id)).toBe('terminal-wrapper');
    });
  }
