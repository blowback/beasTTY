// VT52 character ROM, extracted from an original DEC VT52 terminal.
// Source: data/vt52rom.h (128 chars × 8 rows × 1 byte per row, 7-bit, bit 6 = leftmost).
//
// Storage layout matches BITMAP_FONT in bitmap-font.js so atlas.js can swap
// font tables without changing its iteration shape:
//   128 chars × 16 rows × 1 byte per row = 2048 bytes, MSB-left.
// Each source ROM row is shifted left by 1 (bit 6 → bit 7) and emitted twice
// so the native 8-row glyph fills the existing 16-row cell footprint with
// authentic VT52 chunky scanlines.
//
// Chars 0..31 are the VT52 special graphics character set (enabled by ESC F,
// disabled by ESC G; in graphics mode codepoints 0x60..0x7F render these
// glyphs). They are present in the data but unreachable through normal
// output until ESC F/G is wired in the parser.

const RAW_VT52_ROM = new Uint8Array([
    // 0x00..0x07  (special graphics 0..7)
    ...
    // 0x50..0x5F (P-Z + brackets/underscore)
...
// Expand 8 source rows × 7-bit (bit 6 = leftmost) to 16 output rows × 8-bit
// (bit 7 = leftmost) so the renderer's existing iteration over a 16-row,
// MSB-left glyph table works unchanged. Each ROM row appears twice — the
// authentic VT52 chunky scanline look the existing renderer would have
// produced anyway via its 2× CSS-pixel upscale.
function expandRomToRendererFootprint(raw) {
    const out = new Uint8Array(128 * 16);
    for (let ch = 0; ch < 128; ch++) {
        for (let r = 0; r < 8; r++) {
            const src = (raw[ch * 8 + r] << 1) & 0xFF;   // bit 6 → bit 7 (leftmost)
            out[ch * 16 + r * 2 + 0] = src;
            out[ch * 16 + r * 2 + 1] = src;
        }
    }
...
export const VT52_ROM_FONT = expandRomToRendererFootprint(RAW_VT52_ROM);
